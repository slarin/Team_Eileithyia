                ...
                <div class="container-liquid">
                    <div class="row">
                        <div class="col-xs-12">
                            <div class="sec-box">
                                <header>
                                    <h2 class="heading">Dealers (<?php echo $dealer_num ?>)</h2>
                                </header>
                                                ...
                                                if($coys)
                                                {
                                                    foreach ($coys as $val)
                                                    {
                                                        switch ($val->status)
                                                        {
                                                            case 0:
                                                                $sta = "Not Verified";
                                                                $lnk = '<input type="button"  onclick=activate_dealer('.$val->dealer_id.') class="btn btn-danger style2" value="Verify Dealer" />';
                                                                break;
                                                            
                                                            case 1:
                                                                $sta = "Verified";
                                                                $lnk = '<input type="button" onclick=deactivate_dealer('.$val->dealer_id.') class="btn btn-success style2" value="Deactivate Dealer" />';
                                                                break;
                                                            
                                                            case 3:
                                                                $sta = "Suspended";
                                                                $lnk = '<input type="button" onclick=deactivate_dealer('.$val->dealer_id.') class="btn btn-success style2" value="Unsuspend" />';
                                                                break;
                                                        }
...
//                                                                . '<td>'.$state['state'].'</td>'
                                                                ...
                                                                . '<td>'.$sta.'</td>'
                                                                . '<td><a href="'.  base_url().'dealers/upload/'.$val->verify_image.'" target="_blank"><img src="'.base_url().'assets/admin/images/view.png" alt="View" /></a></td>'
